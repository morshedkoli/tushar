...
export default function ReceivablesPage() {
  ...
  const loadDues = async () => {
    const res = await fetch("/api/dues?type=RECEIVABLE&status=OPEN", { cache: "no-store" });
  ...
  const ensureContactId = async (name: string): Promise<number | null> => {
    ...
    try {
      ...
      // refresh cache for subsequent operations
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/dues", {
      ...
      body: JSON.stringify({ type: "RECEIVABLE", amount: Number(amount), description, date, contactId: id }),
    });
    if (res.ok) {
  ...
  const settle = async (id: number) => {
    ...
    const res = await fetch(`/api/dues/${id}/settle`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ accountId: Number(settleAccountId), categoryId: Number(settleCategoryId), date: settleDate, description: "Receivable settled", amount: amt }),
    });
    if (res.ok) {
      setSettleRow(null);
      setSettleAmount("");
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 space-y-6 pt-16 lg:pt-8">
      <header className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Receivables</h1>
      ...
      <form onSubmit={submit} className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6">
        <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-4">Add Receivable</h2>
            ...
            <tbody>
              {dues.map(d => (
                <tr key={d.id} className="border-b border-gray-100">
                  ...
                  <td className="px-4 py-3 font-semibold text-sm text-green-700">৳{d.amount.toFixed(2)}</td>
              ...
              {dues.length === 0 && (
                <tr>
                  <td className="px-4 py-12 text-center text-gray-400 text-sm" colSpan={5}>No receivables</td>
                </tr>
              )}
            </tbody>
          </table>
